...
from .model.Model import ConstrainedPosteriorMean
from .samplers.samplers import quantileSampler
...
def acquisition_function_factory(type, model, objective, best_value, idx, number_of_outputs, penalty_value, iteration,
    ...
    if type is AcquisitionFunctionType.BOTORCH_EXPECTED_IMPROVEMENT:
        ...
        return DecoupledConstrainedKnowledgeGradient(model, sampler=sampler_list, num_fantasies=5,
                                                     objective=objective,
                                                     number_of_raw_points=80,  # can make smaller if speed
                                                     number_of_restarts=5,  # this too, not too small though
                                                     seed=iteration,
        ...
        return DecoupledConstrainedKnowledgeGradient(model, sampler=sampler_list, num_fantasies=5,
                                                     objective=objective,
                                                     number_of_raw_points=2,  # can make smaller if speed
                                                     number_of_restarts=2,  # this too, not too small though
                                                     seed=iteration,
                                                     # play with this if it gets too slow...get it down a little...
                                                     X_evaluation_mask=x_eval_mask,
                                                     penalty_value=penalty_value,
